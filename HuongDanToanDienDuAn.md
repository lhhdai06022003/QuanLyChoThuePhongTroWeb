...
## DỰ ÁN QUẢN LÝ CHO THUÊ PHÒNG TRỌ (QUANTLYCHOTHUEPHONGTROW)
...
├── Services/                     <-- Dịch vụ dùng chung hệ thống (ví dụ: MenuService)
├── wwwroot/                      <-- Thư mục tài nguyên tĩnh (css, js, hình ảnh, Tabler Theme)
├── appsettings.json              <-- Cấu hình kết nối DB và cấu hình tài khoản VietQR
...
2.  **Đăng ký Dependency Injection (DI)**: Khi tạo mới một cặp Interface/Service, bắt buộc phải đăng ký trong `Program.cs` dưới dạng Scoped (Ví dụ: `builder.Services.AddScoped<IPhongTroService, PhongTroService>();`).
...
*   **Giao diện xuất hiện**: Truy cập từ Menu: **Quản lý nhà trọ** -> **Hợp đồng**. Popup thành viên ở ghép xuất hiện khi click vào nút "Thành viên" của hợp đồng.
*   **Cách thức hoạt động**:
    ...
    *   *Quản lý thành viên*: Khi thêm thành viên vào ở ghép phòng, hệ thống đếm số người ở thực tế (bao gồm khách đại diện + thành viên cũ đang ở) so khớp với sức chứa tối đa của phòng (`SoNguoiToiDa`). Nếu vượt quá, hệ thống sẽ chặn hành động.
...
*   **Cách thức hoạt động**:
    ...
    *   Khi tính hóa đơn hàng tháng, hệ thống sẽ dò tìm đơn giá của dịch vụ tại chi nhánh đó để áp giá tính tiền cho phòng sử dụng.
...
*   **Cách thức hoạt động**:
    ...
    *   *Gửi Email tự động*: Tự động gửi email thông báo kèm đính kèm tệp PDF hóa đơn vừa được sinh ra cho địa chỉ email khách thuê phòng thông qua giao thức SMTP.
...
2.  **Cấu hình kết nối**: Mở file `appsettings.json`, chỉnh sửa chuỗi kết nối tại mục `ConnectionStrings:DefaultConnection` trùng với thông tin Host, Port, Username và Password của PostgreSQL trên máy bạn.
3.  **Khởi tạo Database**:
